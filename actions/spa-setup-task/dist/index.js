...
exports.ingressAnnotations = ingressAnnotations;
function trimRight(s, char) {
...
exports.serviceForApp = serviceForApp;
...
exports.ingressForApp = ingressForApp;
...
const spa_1 = __nccwpck_require__(4589);
function run() {
...
function validateInputs(team, app, ingress, ingressClass, environment) {
    ...
    if (!isValidAppName(app)) {
        return Error(`SPADEPLOY-002: Invalid app name: ${app}. App name must match regex: ^[a-z0-9]([-a-z0-9]*[a-z0-9])?$`);
    }
    if (!isValidAppName(environment)) {
        return Error(`SPADEPLOY-003: Invalid environment name: ${environment}. Environment name must match regex: ^[a-z0-9]([-a-z0-9]*[a-z0-9])?$`);
    }
    if (ingress.length === 0) {
...
exports.spaSetupTask = spaSetupTask;
