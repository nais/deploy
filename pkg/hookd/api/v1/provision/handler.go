...
func (h *Handler) TeamApiKey(w http.ResponseWriter, r *http.Request) {
	fields := middleware.RequestLogFields(r)
	logger := log.WithFields(fields)

	team := chi.URLParam(r, "team")

	keys, err := h.APIKeyStorage.ApiKeys(r.Context(), team)
	if err != nil {
...
func (h *Handler) RotateTeamApiKey(w http.ResponseWriter, r *http.Request) {
	fields := middleware.RequestLogFields(r)
	logger := log.WithFields(fields)
	...
	if err := h.APIKeyStorage.RotateApiKey(r.Context(), team, key); err != nil {
		w.WriteHeader(http.StatusInternalServerError)
